...
/**
 * Clase simple con utilidades básicas para entrada de información por
 * consola.
...
public class UtilConsola {
     ...
     * @param repetirMientrasVacio Si es false, permitirá cadenas vacias como entrada.
     * Si es true, no permitirá cadenas vacías, obligará a introducir una.
     * Una cadena vacía se consigue simplemente pulsando Enter.
     * @param mensaje Pequeño trozo de texto que se mostrará al usuario para indicar
     * que debe introducir.
     ...
     * @param mensaje Pequeño trozo de texto que se mostrará al usuario para indicar
     * que debe introducir.
     * @param opciones Permite indicar la opción que el usario tendrá que introducir.
     * Se indica de la siguiente forma "entrada=valor a retornar,entrada2=valor a retornar".
     * Por ejemplo: "s=SI,si=SI,n=NO,no=NO".
     * @param ignoreCase Permite indicar si se deben considerar las mayusculas o no.
     * Si es true, se ignoran.
     * @return Retornará la cadena, o null si el usuario no ha introducido
     * ningún dato.
     */
    public static String leerOpciones(String mensaje, String opciones, boolean repetirMientrasVacio,boolean ignoreCase) {
        ...
        for (int i=0; i<splittedOptions.length;i++) 
        {
            ...
            if (ignoreCase) value[0]=value[0].toLowerCase();
            if (value.length>1) t.put(value[0].trim(), value[1].trim());
            else t.put(value[0].trim(),"NULL");           
        }        
        do
        {
            if (cadena != null) {
            ...
            System.out.print(mensaje);
            cadena = inScanner.nextLine().trim();
            if (ignoreCase) cadena=cadena.toLowerCase();
        }
        while (repetirMientrasVacio && !t.containsKey(cadena));
        
        if (t.containsKey(cadena)) cadena = t.get(cadena);
        else cadena = null;        
        return cadena;
    }
         
    /**
     * Lee un entero desde consola. Si no puede leer el entero o se
     * produce un error, entonces retorna null.
     * @param mensaje Pequeño trozo de texto que se mostrará al usuario para indicar
     * que debe introducir.
    ...
    {        
        ...
        System.out.print(mensaje);
        try {
             opt=Integer.parseInt(inScanner.nextLine().trim());
        }
        catch (Exception o)
        {
            opt=null;
    ...
    /**
     * Lee un decimal desde consola. Si no puede leer el decimal o se
     * produce un error, entonces retorna null.
     * @param mensaje Pequeño trozo de texto que se mostrará al usuario para indicar
     * que debe introducir.
     * @param RepetirHastaCorrecto Si este parámetro es true se pedirá una y
    ...
    {        
        ...
        System.out.print(mensaje);
        try {
             opt=Float.parseFloat(inScanner.nextLine().trim());
        }
        catch (Exception o)
        {
            opt=null;
     ...
     /**
     * Lee una fecha desde consola. Si no puede leer la fecah o se
     * produce un error, entonces retorna null.
     * @param mensaje Pequeño trozo de texto que se mostrará al usuario para indicar
     * que debe introducir.
     * @param RepetirHastaCorrecto Si este parámetro es true se pedirá una y
    ...
    public static Date leerFecha (String mensaje, boolean RepetirHastaCorrecto)
    {
        ...
        System.out.print(mensaje);
        try {
             d=sdf.parse(inScanner.nextLine().trim());
        }
        catch (Exception o)
        {
